...
class AdminBillingController extends AdminBaseController
{
    ...
    public function index(Request $request)
    {
        ...
        $offline = DB::table('offline_invoices')
            ...
            ->selectRaw('offline_invoices.id , "Offline" as method, offline_invoices.pay_date as paid_on, "" as end_on ,offline_invoices.next_pay_date, offline_invoices.created_at')
        ...
        if ($this->firstInvoice) {
            if ($this->firstInvoice->next_pay_date) {
                ...
                if ($this->firstInvoice->method == 'Offline') {
                    $this->nextPaymentDate = Carbon::parse($this->firstInvoice->next_pay_date)->toFormattedDateString();
                }
            }
...
//        return json_encode($this->firstInvoice);
        ...
        return DataTables::of($paypalData)
            ...
            ->addColumn('action', function ($row) {
                ...
                if ($row->method == 'Stripe' && $row->invoice_id) {
                    $btn =  '<a href="' . route('admin.stripe.invoice-download', $row->invoice_id) . '" class="btn btn-primary btn-circle waves-effect" data-toggle="tooltip" data-original-title="Download"><span></span> <i class="fa fa-download"></i></a>';
                }

                if ($row->method == 'Paypal') {
                    $btn =  '<a href="' . route('admin.paypal.invoice-download', $row->id) . '" class="btn btn-primary btn-circle waves-effect" data-toggle="tooltip" data-original-title="Download"><span></span> <i class="fa fa-download"></i></a>';
                }

                if ($row->method == 'Razorpay') {
                    $btn =  '<a href="' . route('admin.billing.razorpay-invoice-download', $row->id) . '" class="btn btn-primary btn-circle waves-effect" data-toggle="tooltip" data-original-title="Download"><span></span> <i class="fa fa-download"></i></a>';
                }

                if ($row->method == 'Offline') {
                    $btn =  '<a href="' . route('admin.billing.offline-invoice-download', $row->id) . '" class="btn btn-primary btn-circle waves-effect" data-toggle="tooltip" data-original-title="Download"><span></span> <i class="fa fa-download"></i></a>';
                }

                if ($row->method == 'Paystack') {
                    $btn =  '<a href="' . route('admin.billing.paystack-invoice-download', $row->id) . '" class="btn btn-primary btn-circle waves-effect" data-toggle="tooltip" data-original-title="Download"><span></span> <i class="fa fa-download"></i></a>';
                }

                if ($row->method == 'Mollie') {
                    $btn =  '<a href="' . route('admin.billing.mollie-invoice-download', $row->id) . '" class="btn btn-primary btn-circle waves-effect" data-toggle="tooltip" data-original-title="Download"><span></span> <i class="fa fa-download"></i></a>';
                }

                if ($row->method == 'Authorize') {
                    $btn =  '<a href="' . route('admin.billing.authorize-invoice-download', $row->id) . '" class="btn btn-primary btn-circle waves-effect" data-toggle="tooltip" data-original-title="Download"><span></span> <i class="fa fa-download"></i></a>';
                }
    ...
    public function cancelAccount(){
        $company = company();
        $company->package_id = Package::where('name','Gratuit (15 jours)')->first()->id;
        $company->save();
    ...
    public function offlinePaymentSubmit(OfflinePaymentRequest $request)
    {
        ...
        $offlineInvoice->name = $request->type;
        $offlineInvoice->fname = $request->fname;
        $offlineInvoice->iban = $request->iban;
        $offlineInvoice->bic = $request->bic;
        $offlineInvoice->bank = $request->bank;
        $offlineInvoice->agency = $request->agency;
        ...
        $data = array('invoice'=>$offlineInvoice);
        Mail::send('offline-invoice.bank-deposit', $data, function($message) use ($request,$offlinePlanChange) {
            $message->to('info@visioproact.com', config('app.name'))->subject
            ('Bank Deposit Payment Submitted' . ' - ' . config('app.name'));
            $message->attach(public_path('user-uploads/offline-payment-files/'.$offlinePlanChange->file_name));
        });
