...
class SuperAdminInvoiceController extends SuperAdminBaseController
{
    ...
    public function index()
    {
        ...
        $stripeInvoicesAmount = DB::table('stripe_invoices')
            ->whereNotNull('stripe_invoices.pay_date')->sum('amount');

        $razorpayInvoiceAmount = DB::table('razorpay_invoices')
            ->whereNotNull('razorpay_invoices.pay_date')->sum('amount');

        $PaypalInvoicesAmount = DB::table('paypal_invoices')
            ->where('paypal_invoices.status', 'paid')->sum('amount');

        $paystackInvoicesAmount = DB::table('paystack_invoices')
            ->whereNotNull('paystack_invoices.pay_date')->sum('amount');

        $mollieInvoicesAmount = DB::table('mollie_invoices')
            ->whereNotNull('mollie_invoices.pay_date')->sum('amount');

        $authorizeInvoicesAmount = DB::table('authorize_invoices')
            ->whereNotNull('authorize_invoices.pay_date')->sum('amount');

        $offlineInvoicesAmount = DB::table('offline_invoices')
            ->whereNotNull('offline_invoices.pay_date')->sum('amount');
        ...
        $this->totalInvoicesAmount = ($stripeInvoicesAmount + $razorpayInvoiceAmount + $PaypalInvoicesAmount + $paystackInvoicesAmount + $mollieInvoicesAmount + $authorizeInvoicesAmount + $offlineInvoicesAmount);
    ...
    public function data()
    {
        ...
        $offline = DB::table('offline_invoices')->join('packages', 'packages.id', 'offline_invoices.package_id')
            ->join('companies', 'companies.id', 'offline_invoices.company_id')
//            ->join('offline_payment_methods', 'offline_payment_methods.id', 'offline_invoices.offline_method_id')
...
//            })
            ...
            ->editColumn('method', function ($row) {
                if($row->method == 'Offline' && isset($row->offline_payment_method)) {
