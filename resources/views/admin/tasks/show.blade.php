...
<div class="r-panel-body p-t-0">
    ...
    <div class="row">
        <div class="col-xs-12 col-md-9 p-t-20 b-r h-scroll d-grid" >
            <div class="col-xs-12">
                <a href="{{route('front.task-share',[$task->hash])}}" target="_blank" data-toggle="tooltip" data-placement="bottom"
                   data-original-title="@lang('app.share')" class="btn btn-default btn-sm btn-rounded btn-outline pull-right m-l-5"> <i class="fa fa-share-alt"></i></a>
                @php $pin = $task->pinned() @endphp

                <a href="javascript:;" class="btn btn-cs-blue @if(!$pin) btn-outline @endif pull-right m-l-5" data-placement="bottom"  data-toggle="tooltip" data-original-title="@if($pin) @lang('app.unpin') @else @lang('app.pin') @endif"    data-pinned="@if($pin) pinned @else unpinned @endif" id="pinnedItem" >
                    <i class="icon-pin icon-2 pin-icon  @if($pin) pinned @else unpinned @endif" ></i>
                </a>

                <a href="{{route('admin.all-tasks.edit',$task->id)}}" class="btn btn-default btn-sm btn-rounded btn-outline pull-right m-l-5"> <i class="fa fa-edit"></i> @lang('app.edit')</a>

                <a href="javascript:;" id="completedButton" class="btn btn-success btn-sm btn-rounded @if($task->board_column->slug == 'completed') hide @endif "  onclick="markComplete('completed')" ><i class="fa fa-check"></i> @lang('modules.tasks.markComplete')</a>

                <a href="javascript:;" id="inCompletedButton" class="btn btn-default btn-outline btn-sm btn-rounded @if($task->board_column->slug != 'completed') hidden @endif"  onclick="markComplete('incomplete')"><i class="fa fa-times"></i> @lang('modules.tasks.markIncomplete')</a>

                <a href="javascript:;" id="reminderButton" class="btn btn-default btn-sm btn-rounded btn-outline pull-right @if($task->board_column->slug == 'completed') hidden @endif" title="@lang('messages.remindToAssignedEmployee')"><i class="fa fa-bell"></i> @lang('modules.tasks.reminder')</a>

                @if ($task->board_column->slug != 'completed')
                    @if (count($task->activeTimerAll) > 0)
                        <a href="javascript:;" class="btn btn-default btn-sm btn-rounded btn-outline pull-right  m-l-5 timer-modal"><i class="fa fa-clock-o"></i> @lang('modules.projects.activeTimers')
            ...
            <div class="tab-content" id="task-detail-section">
                ...
                <div role="tabpanel" class="tab-pane" id="profile1">
                    ...
                    <div class="col-xs-12 m-t-5 ">
                        <h5><i class="ti-check-box"></i> @lang('modules.tasks.subTask')<span class="pull-right" id="percentage-count"><span class="donut" data-peity='{ "fill": ["#00c292", "#eeeeee"],    "innerRadius": 5, "radius": 8 }'>  @if (count($task->subtasks) > 0) {{ count($task->completedSubtasks) }}/{{ count($task->subtasks) }} @else 0/0 @endif</span> <span class="text-muted font-12"> @if (count($task->subtasks)){{ floor((count($task->completedSubtasks)/count($task->subtasks))*100) }}% @else 0% @endif</span></span></h5>
                        <ul class="list-group b-t" id="sub-task-list">
                            @foreach($task->subtasks as $subtask)
                                @php $subTaskCount = $subtask->files->count(); @endphp
                                <li class="list-group-item subTaskFileDiv{{$subtask->id}} @if($subTaskCount > 0) nonBottomBorder @endif" >
                                    <div class="col-xs-9">
                ...
                <div role="tabpanel" class="tab-pane" id="messages1">
                    <div class="col-xs-12">
                        <button href="javascript:;" id="show-dropzone" class="btn btn-success btn-sm"><i class="ti-upload"></i> @lang('modules.projects.uploadFile')</button>
                ...
                <div role="tabpanel" class="tab-pane" id="settings1">
                    ...
                    <div class="col-xs-12">
                        ...
                        class="btn btn-cs-green m-t-10 m-b-20"><i class="ti-upload"></i> @lang('modules.projects.uploadImage')</button>
                        ...
                        <div class="col-xs-3">
                            <a href="javascript:;" id="submit-comment" class="btn btn-cs-blue"><i class="fa fa-send"></i> @lang('app.submit')</a>
                        </div>
                    </div>
                ...
                <div role="tabpanel" class="tab-pane" id="notes1">
                    ...
                    <div class="form-group" id="note-box">
                        ...
                        <div class="col-xs-3">
                            <a href="javascript:;" id="submit-note" class="btn btn-cs-blue m-t-20"><i class="fa fa-send"></i> @lang('app.submit')</a>
                        </div>
                    </div>
                ...
                <div role="tabpanel" class="tab-pane" id="history1">
                    ...
                    <div class="col-xs-12" id="task-history-section" style="width: 100%;">
                    </div>
                </div>
            </div>
        </div>
