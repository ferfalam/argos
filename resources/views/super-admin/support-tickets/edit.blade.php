...
@section('page-title')
    <div class="row bg-title">
        ...
        <div class="col-lg-7 col-md-4 col-sm-4 col-xs-12 bg-title-left">
            <h4 class="page-title" style="min-width: max-content"> {{ __($pageTitle) }} @lang('app.requestedBy') (
                {{ $ticket->requester->company->company_name }} )</h4>
        </div>
        <!-- /.page title -->
        <!-- .breadcrumb -->
                ...
                <select class="form-control select2" name="agent_id" id="agent_id" data-style="form-control">
                    @forelse($superadmins as $superadmin)
                        <option value="{{ $superadmin->id }}">
                            {{ ucwords($superadmin->name) . ' [' . $superadmin->email . ']' }}</option>
                    @empty
        ...
        <div class="col-md-6">
            <div class="form-group">
                <label class="control-label">@lang('modules.invoices.type') <a class="btn btn-xs btn-info btn-outline"
                        href="javascript:;" id="add-type"><i class="fa fa-plus"></i> </a></label>
                <select class="form-control selectpicker add-type" name="type_id" id="type_id" data-style="form-control">
                    @forelse($types as $type)
                        <option @if ($type->id == $ticket->support_ticket_type_id)
                            selected
                    @endif
...
@section('content')
    ...
    <div class="form-body">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-12 m-b-10">
                            ...
                            <div class="font-12">
                                ...
                                &bull; {{ ucwords($ticket->requester->name) . ' <' . $ticket->requester->email . '>' }}</div>
                        </div>
                                ...
                                <div class="row m-b-5">
                                    ...
                                    <div class="col-xs-8 col-md-10">
                                        <h5 class="m-t-0 font-bold">
                                            <a href="#" class="text-inverse">{{ ucwords($reply->user->name) }}
                                                <span
                                                    class="text-muted font-12 font-normal">{{ $reply->created_at->timezone($global->timezone)->format($global->date_format . ' ' . $global->time_format) }}</span>
                                            </a>
                                        </h5>
                                                ...
                                                <li class="list-group-item b-none col-md-6">
                                                    <div class="row">
                                                        ...
                                                        <div class="col-md-4 text-right">
                                                            ...
                                                            <span
                                                                class="clearfix font-12 text-muted">{{ $file->created_at->diffForHumans() }}</span>
                                                        </div>
                                                    </div>
                                                </li>
                                            @empty
                            ...
                            <div class="row m-b-20">
                                <div class="col-xs-12">
                                    <button type="button"
                                        ...
                                        style="margin-bottom: 10px;display: none "><i class="fa fa-upload"></i> File Select Or
                                        Upload</button>
        ...
        {{-- Ajax Modal --}}
        <div class="modal fade bs-modal-md in" id="ticketModal" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
            ...
            $('.submit-ticket').click(function() {
                ...
                $.easyAjax({
                    ...
                    success: function(response) {
                        ...
                        if (status == 'open')
                            $('#ticket-status').html('<label class="label label-danger">' + status +
                                '</label>');
                        else if (status == 'pending')
                            $('#ticket-status').html('<label class="label label-warning">' + status +
                                '</label>');
                        else if (status == 'resolved')
                            $('#ticket-status').html('<label class="label label-info">' + status +
                                '</label>');
                        else if (status == 'closed')
                            $('#ticket-status').html('<label class="label label-success">' + status +
                                '</label>');
