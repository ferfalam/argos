...
@section('page-title')
    <div class="row bg-title">
        ...
        <div class="col-lg-6 col-sm-8 col-md-8 col-xs-12 text-right bg-title-right">
            <a href="javascript:;"
                class="btn btn-outline btn-success btn-sm addFeature pull-right">@lang('modules.featureSetting.addFeature')
                ...
                <div class="vtabs customvtab">
                    ...
                    <div class="tab-content p-t-0">
                        <div id="vhome3" class="tab-pane active">
                            <div class="row">
                                <div class="col-sm-12">
                                    <h4 class="box-title m-b-10">@lang('modules.feature.setting') @lang('app.for') -
                                        <b>{{ $frontFeature->title }}</b></h4>
                                            ...
                                            <tbody>
                                                @forelse($frontFeature->features as $feature)
                                                    <tr>
                                                        ...
                                                        <td>{{ $feature->language ? $feature->language->language_name : 'English' }}
                                                        </td>
                                                        <td @if ($feature->type != 'image' && $feature->type != 'apps') style="font-size: 27px" @endif>
                                                            @if ($feature->type == 'image' || $feature->type == 'apps')
                                                                <img height="100" width="85"
                                                                    src="{{ $feature->image_url }}" alt="" />
                                                            @else
                                                                <i class="{{ $feature->icon }}"></i>
                                                            @endif
                                                        </td>
                                                        <td class="text-nowrap">
                                                            <a href="javascript:;" data-feature-id="{{ $feature->id }}"
...
@push('footer-script')
    <script>
        ...
        $('.editFeature').click(function() {
            ...
            var url =
                "{{ route('super-admin.feature-settings.edit', ':id') }}?featureId={{ $frontFeature->id }}&type=icon";
        ...
        $('.addFeature').click(function() {
            var url =
                "{{ route('super-admin.feature-settings.create') }}?featureId={{ $frontFeature->id }}&type=icon";
            $('#modelHeading').html('Currency Convert Key');
            $.ajaxModal('#projectCategoryModal', url);
        })
