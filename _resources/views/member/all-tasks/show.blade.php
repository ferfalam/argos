    ...
    <div class="row">
        ...
        <div class="col-xs-12 col-md-9 p-t-20 b-r h-scroll">
            ...
            <div class="col-xs-12 d-flex">
                ...
                <a href="javascript:;" id="completedButton" class="btn btn-cs-green  m-b-10 btn-rounded @if($task->board_column->slug == 'completed') hidden @endif "  onclick="markComplete('completed')" ><i class="fa fa-check"></i> @lang('modules.tasks.markComplete')</a>
                ...
                <a href="javascript:;" class="btn btn-sm btn-rounded m-b-10 btn-cs-blue @if(!$pin) btn-outline @endif "   data-placement="bottom"  data-toggle="tooltip" data-original-title="@if($pin) @lang('app.unpin') @else @lang('app.pin') @endif"  data-pinned="@if($pin) pinned @else unpinned @endif" id="pinnedItem" >
                ...
                @if ($task->board_column->slug != 'completed')
                    @if (is_null($task->activeTimer) && !is_null($task->is_task_user))
                        <a href="javascript:;" id="start-task-timer"  class="btn btn-default  btn-sm m-b-10 btn-rounded btn-outline pull-right m-r-5"> <i class="fa fa-clock-o"></i> @lang('modules.timeLogs.startTimer')</a>
                    @elseif (!is_null($task->activeTimer) && !is_null($task->is_task_user))
                        ...
                        data-original-title="@lang('app.stop')" data-time-id="{{ $task->activeTimer->id }}" class="btn btn-default  btn-sm m-b-10 btn-rounded btn-outline pull-right m-r-5 w-100"> <span id="active-task-timer" class="b-r m-r-5 p-r-5">{{ $task->activeTimer->timer }}</span> <i class="fa fa-stop text-danger"></i></a>
                    @endif
                @endif
            </div>
